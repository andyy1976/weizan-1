	...
	require(['angular', 'bootstrap', 'utilsms'], function(angular, $, u){
		angular.module('app', []).controller('loginPanel', function($scope, $http){
			...
			$scope.login = function(){
				...
				if(ret[mode].realname == '' || ret[mode].username == ''<?php  if($config['smsstatus']) { ?> || ret[mode].password == ''<?php  } ?>) {
...
<style>
	body{background:<?php  echo $config['memberbgcolor'];?>;}
	.panel{margin:.5em; border:none;}
	.panel-info>.panel-heading {background: -webkit-gradient(linear, 0 0, 100% 0, from(#D7D7D7), to(#D7D7D7), color-stop(30%, <?php  echo $config['memberbgcolor'];?>)); color:#666666; border:none;}
	...
	.btn.btn-primary{background: <?php  echo $config['memberbntcolor'];?>; color: <?php  echo $config['memberbnttcolor'];?>; border: 0;}
</style>
<?php  if($this->module['config']['module_branch']) { ?>
...
<div class=" panel panel-info ng-container" ng-controller="loginPanel">
	<div class="panel-heading">
		<h4><?php  echo $_W['account']['childname'];?>确认身份</h4>
	</div>
		...
		<div class="tab-content">
			<div class="tab-pane animated active" id="code">
				<div class="form-group has-feedback" ng-class="{'has-error': label.code.error}">
					...
					<input ng-model="ret.code.realname" type="text" class="form-control" placeholder="真实姓名" ng-init="ret.code.realname='<?php  echo $profile['realname'];?>'" ng-value="">
					<span class="glyphicon glyphicon-remove form-control-feedback" ng-show="label.code.error"></span>
				</div>
				<div class="form-group has-feedback" ng-class="{'has-error': label.code.error}">
					<label class="control-label sr-only"></label>
					<input ng-model="ret.code.username" type="tel" class="form-control" placeholder="手机号" ng-init="ret.code.username='<?php  echo $profile['mobile'];?>'" ng-value="">
					<span class="glyphicon glyphicon-remove form-control-feedback" ng-show="label.code.error"></span>
				</div>
				<?php  if($config['smsstatus'] && $member_sms['status']) { ?>
