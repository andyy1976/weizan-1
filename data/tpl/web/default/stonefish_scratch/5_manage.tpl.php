...
<div class="main">
	<ul class="nav nav-tabs">
		<li<?php  if($_GPC['do'] == 'manage' || $_GPC['do'] == '' ) { ?> class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('manage');?>"> 管理幸运刮刮卡</a></li>
		...
		<table class="table table-hover">
			...
			<tbody>
				...
				<tr>
					...
					<td><a href="<?php  echo url('platform/reply/post',array('m'=>'stonefish_scratch','rid'=>$row['rid']));?>" title="<?php  echo $row['title'];?>"><?php  echo $row['title'];?></a></td>
					<td><a href="<?php  echo url('site/entry/trend',array('m'=>'stonefish_scratch','rid'=>$row['rid']));?>" title="活动分析表"><?php  echo $row['fansnum'];?>/<?php  echo $row['viewnum'];?></a></td>
					...
					<td>
                        ...
                        <a class="btn btn-default" data-toggle="tooltip" data-placement="top" href="<?php  echo url('platform/reply/post',array('m'=>'stonefish_scratch','rid'=>$row['rid']));?>" title="编辑"><i class="fa fa-edit"></i></a>
                        <?php  if($row['isshow']==0) { ?>
	...
	$("input[name=deleteall]").click(function(){
       ...
       // if( confirm("确认要删除选择的记录?")){
		...
		$.post('<?php  echo $this->createWebUrl('deleteAll')?>', {idArr:id},function(data){
			if (data.errno ==0){
				u.message(data.error, '<?php  echo url('site/entry/manage',array('m' => 'stonefish_scratch'))?>', 'warning');
			}else{
