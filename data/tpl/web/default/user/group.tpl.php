	...
	<form action="" method="post"  class="form-horizontal" role="form" enctype="multipart/form-data" id="form1">
		...
		<div class="panel panel-default">
			...
			<div class="panel-body table-responsive">
				...
				<div class="form-group">
					<label class="col-xs-12 col-sm-2 col-md-2 col-lg-2 control-label">充值比例</label>
					<div class="col-sm-10 col-xs-12">
						<input type="text" class="form-control" name="discount" value="<?php  echo $group['discount'];?>" />
						<span class="help-block">用户充值余额时的比率。例如如果为2，则用户支付100元可以购买到200余额，余额可以用来续费和购买模块</span>
					</div>
				</div>
				<div class="form-group">
			        <label class="col-xs-12 col-sm-2 col-md-2 col-lg-2 control-label">是否开启版权设置</label>
			        <div class="col-sm-10 col-xs-12">
				        <label for="showhomepage_1" class="radio-inline"><input type="radio" name="domain" value="1" id="showhomepage_1" <?php  if(!empty($group['domain'])) { ?> checked<?php  } ?>> 是</label>&nbsp;&nbsp;&nbsp;
				        <label for="showhomepage_2" class="radio-inline"><input type="radio" name="domain" value="0" id="showhomepage_2" <?php  if(empty($group['domain'])) { ?> checked<?php  } ?>> 否</label>
				        <span class="help-block">设置为‘是’，该用户组下的用户将可设置独立域名和站点版权信息。</span>
			        </div>
		        </div>
				<div class="form-group">
					<label class="col-xs-12 col-sm-2 col-md-2 col-lg-2 control-label">用户组年费</label>
					<div class="col-sm-10 col-xs-12">
						<input type="text" class="form-control" name="price" value="<?php  echo $group['price'];?>" />
