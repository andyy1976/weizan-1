...
<form action="" class="form-horizontal form" method="post" enctype="multipart/form-data" id="form1">
		...
		<div class="panel panel-default">
		    <div class="panel-heading">
			    是否启用短信验证：
			    <input type="checkbox" name="flag" value="1" <?php  if(intval($setting['smsstatus'])==1) { ?> checked="checked" <?php  } ?> data="<?php  echo $setting['id'];?>"/>
		    </div>
			...
			<div class="row-fluid">
    			<div class="span8 control-group">
					...
					<?php  if($_W['role']=='founder') { ?><a style="margin-left:10px;" class="btn btn-danger addsms" href="javascript:void(0)"><i class="fa fa-plus"></i> 短信充值</a><?php  } ?>
       		 </div>
    		</div>
    		</div>
	    </div>		
		<?php  if(($setting['smsstatus'] != 0)) { ?>
		<div class="panel panel-default" id="cardmain">
			...
			<div class="panel-body">
				<div class="form-group">
					...
					<div class="col-sm-9">
						<label for="smstype0" class="radio-inline"><input type="radio" name="smstype" value="0" id="smstype0" <?php  if(intval($setting['smstype'])==0) { ?> checked="checked" <?php  } ?> onclick="$('#juhe').hide();"/> 官方短信(下版本提供)</label>&nbsp;&nbsp;&nbsp;
						<label for="smstype1" class="radio-inline"><input type="radio" name="smstype" value="1" id="smstype1" <?php  if(intval($setting['smstype'])==1) { ?> checked="checked" <?php  } ?> onclick="$('#juhe').show();"/> 聚合短信(请选择此项)</label>
					</div>
				</div>
				<div id="juhe"<?php  if($setting['smstype']==0) { ?> style="display: none;"<?php  } ?>>
				<div class="form-group">
					<label class="col-xs-12 col-sm-3 col-md-2 control-label">短信验证KEY</label>
					<div class="col-sm-9">
						<input type="text" name="smskey" class="form-control" value="<?php  echo $setting['smskey'];?>" />
					</div>
				</div>
				<div class="form-group">
					<label class="col-xs-12 col-sm-3 col-md-2 control-label">短信验证模板</label>
					<div class="col-sm-9">
						<input type="text" name="tpl_id" class="form-control" value="<?php  echo $setting['tpl_id'];?>" />
					</div>
				</div>
				<div class="form-group">
					<label class="col-xs-12 col-sm-3 col-md-2 control-label">短信验证签名</label>
					<div class="col-sm-9">
						<input type="text" name="sign" class="form-control" value="<?php  echo $setting['sign'];?>" />
					</div>
				</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-9 input-group input-medium" style="padding-left:15px;">
                        ...
                        <input type="text" name="aging" value="<?php  echo $setting['aging'];?>" class="form-control">							           
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<label for="totalcnf1" class="radio-inline"><input type="radio" name="agingrepeat" value="0" id="totalcnf1" <?php  if(intval($setting['agingrepeat'])==0) { ?> checked="checked" <?php  } ?>/> 时效内可重复使用</label>&nbsp;&nbsp;&nbsp;
						<label for="totalcnf2" class="radio-inline"><input type="radio" name="agingrepeat" value="1" id="totalcnf2" <?php  if(intval($setting['agingrepeat'])==1) { ?> checked="checked" <?php  } ?>/> 只能使用一次</label>
					</div>
				</div>
			</div>
		</div>
