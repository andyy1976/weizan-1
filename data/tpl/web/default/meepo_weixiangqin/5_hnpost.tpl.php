...
<ul class="nav nav-tabs">
	<li><a href="<?php  echo $this->createWebUrl('display')?>">活动列表</a></li>
	<li<?php  if(!$reid) { ?> class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('post')?>">新建活动</a></li>
	<?php  if($reid) { ?><li class="active"><a href="<?php  echo $this->createWebUrl('post', array('id' => $reid))?>">编辑活动</a></li><?php  } ?>
</ul>
<div class="main">
	<form class="form-horizontal form" action="" method="post" enctype="multipart/form-data" onsubmit="return validate(this);">
    	<div class="panel panel-success">
            <div class="panel-heading">
                新建活动 <span class="text-muted">线下活动是线上的补充，你可以通过活动报名的方式收集粉丝信息</span>
            </div>
            <div class="panel-body">
                ...
                <div class="form-group">
                    ...
                    <div class="col-xs-12 col-sm-9">
                        <textarea  id="content" type="text" class="form-control" placeholder="" name="content" style="height:200px;"><?php  echo $activity['content'];?></textarea>
	        			<span class="help-block">此活动的说明信息. 例如: 请提交你的联系方式, 和要咨询的产品信息. 我们会尽快联系你</span>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-xs-12 col-sm-9">
                            ...
                            <span class="help-block">提交成功以后提示的信息. 例如: 您的信息我们已经收到, 很快会有专人联系你. </span>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-xs-12 col-sm-9">
                         <?php  echo tpl_form_field_image('thumb',$activity['thumb']);?>
	      				<span class="help-block">用一张图片来更好的表现你的主题</span>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-xs-12 col-sm-3 col-md-2 control-label">每人可报名次数</label>
                    <div class="col-xs-12 col-sm-9">
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body table-responsive">
				...
				<span class="help-block" style="color:red">注意: 报名成功启动以后(已经有粉丝用户提交了报名信息), 将不能再修改调查条目, 请仔细编辑. </span>
            </div>
        </div>
            ...
            <div class="modal-body" >  
               <div class="well">
				...
				<div class="well re-value hide">
					...
					<span class="help-block"><strong>设置调查条目的选项(如果有需要的话.) 每行一条记录, 例如: 性别 男, 女</strong></span>
				</div>
...
<script type="text/javascript">
	...
	function validate() {
		if($.trim($(':text[name="activity"]').val()) == '') {
			message('必须填写活动标题.', '', 'error');
			return false;
		}
		if($.trim($('textarea[name="information"]').val()) == '') {
			message('必须填写活动成功提示信息.', '', 'error');
			return false;
		}
		<?php  if(empty($reid)) { ?>
		if($.trim($(':input[name="thumb"]').val()) == '') {
			message('必须上传活动封面.', '', 'error');
			return false;
		}
		<?php  } ?>
		if($(':text[name="title[]"]').length == 0) {
			message('必须设定活动的调查条目.', '', 'error');
			return false;
		}
		var isError = false;
