			...
			<?php  if(!empty($frames)) { ?>
				<div class="col-xs-12 col-sm-3 col-lg-2 big-menu">
					...
					<div class="panel panel-default">
						...
						<ul class="list-group collapse <?php  if($frame['active']) { ?>in<?php  } ?>" id="frame-<?php  echo $k;?>">
				...
				<style>.topNav{border-bottom-color: rgb(0, 0, 0);border-bottom-width: 0.1em;border-bottom-style: inset;}</style>
					<?php  if(CRUMBS_NAV == 1) { ?>
						...
						<ul class="nav navbar-nav">
							<li>
					            <a href="<?php  echo url('home/welcome/ext',array('m'=>'imeepos_runner'))?>"><i class='fa fa-home'></i>小明跑腿</a> 
					        </li>
						    <?php  if(is_array($frames)) { foreach($frames as $k => $frame) { ?>
						    <li class="dropdown <?php  echo $frame['active'];?>">
						        <a href='javascript:;' class="dropdown-toggle" data-toggle="dropdown"><?php  echo $frame['title'];?><i class="fa fa-angle-down"></i></a>
						        <ul class="dropdown-menu">
						        <?php  if(is_array($frame['items'])) { foreach($frame['items'] as $link) { ?>
						            <li class="<?php  echo $link['active'];?>" ><a href="<?php  echo $link['url'];?>" kw="<?php  echo $link['title'];?>"><?php  echo $link['title'];?></a></li>
						        <?php  } } ?>
						        </ul>
						    </li>
							<?php  } } ?>
